...
const firstCat = database.categories && database.categories.length > 0 ? database.categories[0] : null;
const defaultCid = firstCat ? (firstCat.children && firstCat.children.length > 0 ? firstCat.children[0].cid : firstCat.cid) : '4';
const initialProducts = database.products.filter(p => {
  const isOut = p.stock === '0' || p.stock === '缺货';
  return p.cid === defaultCid && !isOut;
  ...
  <section id="digital" class="py-12 relative z-10 font-outfit">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          <h2 class="text-2xl md:text-3xl font-black text-white tracking-tight flex items-center gap-3">
            <span>数字商品现货库</span>
            <span class="text-xs font-mono font-bold text-[#38BDF8] bg-[#0284C7]/20 px-3 py-1 rounded-full border border-[#38BDF8]/40 shadow-[0_0_12px_rgba(56,189,248,0.2)]">2600+ 现货储备</span>
          </h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
        <!-- 左侧类目树折叠栏 (对标 8877) -->
        <div class="hidden md:block col-span-1 space-y-2 max-h-[750px] overflow-y-auto pr-3 scrollbar sticky top-24" id="desktop-categories-sidebar">
          {database.categories.map((cat, idx) => {
            const hasChildren = cat.children && cat.children.length > 0;
            const isFirst = idx === 0;

            return (
              <div class="category-group" data-group-id={cat.cid}>
                {hasChildren ? (
                  <div>
                    <!-- 一级大类卡片（右侧带蓝色胶囊下拉按钮） -->
                    <div class="flex items-center justify-between p-2.5 rounded-2xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 transition-all duration-200">
                      <button 
                        type="button" 
