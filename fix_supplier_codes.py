...
print("=== 1. 升级全局 SKU 净化引擎，拦截所有上游供货代号 (MMO39/ACG-FAKA/DEFAULT) ===")

enhanced_engine = '''
<script is:inline>
window.formatSkuDisplay = function(raw, title, specValues) {
  // 1. 如果带有规格明细属性 (如 race: "美国")，优先展示对应属性
  ...
  // 2. 检测是否属于上游供应商纯代码 (MMO39, ACCSZONE, ACG-FAKA, DEFAULT 等)
  ...
  // 如果属于纯供应商内部代码，自动从商品标题中提取亮点生成专业套餐名
  if (isSupplierId) {
    var tags = [];
...
for root, _, files in os.walk("src"):
    for f in files:
        if f.endswith(".astro"):
            ...
            # 替换或植入最新的 formatSkuDisplay 引擎
            if "window.formatSkuDisplay" in content:
                content = re.sub(r'<script is:inline>[\s\S]*?window\.formatSkuDisplay[\s\S]*?<\/script>', enhanced_engine.strip(), content)
            elif "sku-select-btn" in content:
                content = content + "\n" + enhanced_engine

            # 确保调用 formatSkuDisplay 时传入 (sku.name, (product.title || title), sku.spec_values)
            content = re.sub(
                r'window\.formatSkuDisplay\((sku|item)\.(name|sku_code|skuCode)\)',
                r'window.formatSkuDisplay(\1.name || \1.sku_code || \1.skuCode, (typeof currentProduct !== "undefined" ? currentProduct.title : (typeof product !== "undefined" ? product.title : "")), \1.spec_values)',
                content
            )
